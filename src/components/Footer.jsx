    ...
    <footer className="bg-brand-dark-2 border-t border-white/5 py-12 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-8 items-start">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="relative flex items-center gap-3 text-sm text-white/60 cursor-pointer" onClick={copyEmail}>
            ...
            <button 
              className="ml-1 text-white/40 hover:text-white focus:outline-none transition-colors"
            ...
            {/* Tooltip */}
            <span className={`absolute bottom-full left-1/3 mb-2 px-2 py-1 text-[10px] text-white bg-brand-blue rounded shadow-lg transition-all duration-200 pointer-events-none ${copied ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-1'}`}>
              Email Copied!
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 md:items-end justify-between h-full">
          ...
          <p className="text-white/30 text-xs mt-auto md:text-right">
